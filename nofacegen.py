...
def image_to_single_id_generation_process(
    ...
    single_faceid_cond_kwargs = prepare_single_faceid_cond_kwargs(pil_faceid_image,
                                                                  ...
                                                                  [mix_scale_1, mix_scale_2],face_app )
...
if __name__ == "__main__":  
    ...
    parser.add_argument("--device", type=str, default="1", help="指定要使用的 GPU 设备，例如 '0' 或 '0,1'。")  
    parser.add_argument("--start_row", type=int, default=0, help="CSV 文件中要处理的起始行索引（从 0 开始）。")  
    parser.add_argument("--end_row", type=int, default=9, help="CSV 文件中要处理的结束行索引（包含）。")  
    ...
    parser.add_argument("--prompt_csv", type=str, default="/disk2/fujm/UniPortrait-main/assets/empty.csv", help="提示信息的 CSV 文件路径。")  
    parser.add_argument("--result_dir", type=str, default="/disk2/fujm/UniPortrait-main/testresult/nsfwprompt_noip_conid01", help="保存生成图像的目录。")  
    ...
    # 特定模型路径  
    image_encoder_path = "/disk2/fujm/model/models/image_encoder"  
    ip_ckpt = "/disk2/fujm/model/models/ip-adapter_sd15.bin"  
    face_backbone_ckpt = "/disk2/fujm/face_encoder/glint360k_curricular_face_r101_backbone.bin"  
    uniportrait_faceid_ckpt = "/disk2/fujm/UniPortrait/uniportrait-faceid_sd15.bin"  
    uniportrait_router_ckpt = "/disk2/fujm/UniPortrait/uniportrait-router_sd15.bin"  
   ...
   # 加载 ControlNet 模型  
    ...
    for img_name in tqdm(image_filenames, desc="生成图像"):  
        ...
        # 调用生成函数  
        try:  
            generated_images = image_to_single_id_generation_process(  
                uniportrait_pipeline,  # 将 uniportrait_pipeline 作为参数传入  
                pil_faceid_image=pil_image,  #不闯入faceid模块
                ...
                pil_ip_image=pil_image, ip_scale=0.8,
                prompt=prompt,  
                negative_prompt="",  # 可以根据需要设置  
                num_samples=4,  
                seed=1,  
                image_resolution="512x512",  
                inference_steps=25,  
